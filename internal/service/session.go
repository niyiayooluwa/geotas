...
func (s *SessionService) GetLiveQRToken(ctx context.Context, userID string, sessionID string) (model.QRTokenResponse, error) {
    parsedSessionID, err := parseUUID(sessionID)
    if err != nil {
        return model.QRTokenResponse{}, errors.New("Invalid session_id")
    }

    parsedUserID, err := parseUUID(userID)
    if err != nil {
        return model.QRTokenResponse{}, err
    }
